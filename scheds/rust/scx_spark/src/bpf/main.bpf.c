...
static bool node_rebalance(int node)
{
	...
	/* 
	 * NUMA rebalancing only makes sense for per-node DSQs.
	 * For per-CPU or shared DSQ modes, disable rebalancing.
	 */
	if (dsq_mode != DSQ_MODE_NODE)
		return false;
...
struct task_ctx {
	...
	/*
	 * GPU awareness fields for single-GPU optimization.
	 */
	u32 gpu_workload_type;         /* Type of GPU workload */
	...
	u32 gpu_numa_node;             /* GPU's NUMA node */
	u64 gpu_access_frequency;      /* How often task accesses GPU */
};
...
/*
 * GPU-aware helper functions for single-GPU optimization.
 */

/*
 * Return true if the task is GPU-intensive, false otherwise.
 */
static bool is_gpu_intensive(const struct task_ctx *tctx)
{
	if (!gpu_awareness_enabled || !tctx)
		return false;

	return tctx->gpu_workload_type != GPU_WORKLOAD_NONE;
}

/*
 * Return true if the task is inference workload (latency-sensitive).
 */
static bool is_inference_workload(const struct task_ctx *tctx)
{
	if (!tctx)
		return false;

	return tctx->gpu_workload_type == GPU_WORKLOAD_INFERENCE;
}

/*
 * Return true if the task is training workload (throughput-oriented).
 */
static bool is_training_workload(const struct task_ctx *tctx)
{
	if (!tctx)
		return false;

	return tctx->gpu_workload_type == GPU_WORKLOAD_TRAINING;
}

/*
 * Return true if GPU memory pressure is high.
 */
static bool gpu_memory_pressure_high(void)
{
	if (!gpu_awareness_enabled || gpu_memory_total == 0)
		return false;

	u64 memory_usage_pct = (gpu_memory_used * 100) / gpu_memory_total;
	return memory_usage_pct >= GPU_MEM_PRESSURE_HIGH;
}

/*
 * Return true if CPU is on the same NUMA node as GPU.
 */
static bool is_gpu_local_cpu(s32 cpu)
{
	if (!gpu_awareness_enabled)
		return false;

	int node = __COMPAT_scx_bpf_cpu_node(cpu);
	return node == gpu_numa_node;
}

/*
 * Calculate GPU-aware priority boost for task.
 */
static u64 gpu_priority_boost(const struct task_struct *p, const struct task_ctx *tctx)
{
	if (!gpu_awareness_enabled || !tctx)
		return 0;
	...
	/* Inference workloads get higher priority for latency */
	if (is_inference_workload(tctx)) {
		boost += 1000;  /* Significant boost for inference */
	}

	/* Training workloads get moderate boost */
	if (is_training_workload(tctx)) {
		boost += 500;   /* Moderate boost for training */
	}

	/* Boost based on GPU access frequency */
	if (tctx->gpu_access_frequency > 0) {
		boost += MIN(tctx->gpu_access_frequency / 1000, 200);
	}
...
static u64 task_deadline(const struct task_struct *p, struct task_ctx *tctx)
{
	...
	/*
	 * GPU-aware priority adjustment: Apply priority boost for GPU-intensive tasks.
	 * This reduces the effective deadline for GPU workloads, giving them higher priority.
	 */
	if (gpu_awareness_enabled) {
		u64 gpu_boost = gpu_priority_boost(p, tctx);
		if (gpu_boost > 0) {
			/* Reduce deadline by the boost amount (lower deadline = higher priority) */
			if (tctx->deadline > gpu_boost)
				tctx->deadline -= gpu_boost;
			else
				tctx->deadline = 0;
		}
	}
...
static s32 pick_idle_cpu(struct task_struct *p, s32 prev_cpu, u64 wake_flags, bool *is_idle)
{
	...
	if (smt_enabled) {
		/*
		 * GPU-aware optimization: For GPU-intensive tasks, prioritize
		 * CPUs on the same NUMA node as the GPU to reduce memory access latency.
		 */
		if (is_gpu_intensive(tctx) && is_gpu_local_cpu(prev_cpu)) {
		...
		/*
		 * For GPU-intensive tasks, prioritize full-idle cores on GPU's NUMA node.
		 */
		if (is_gpu_intensive(tctx) && is_gpu_local_cpu(node)) {
			/* Search for full-idle CPU on GPU's NUMA node with L2 cache affinity */
			if (l2_mask) {
				cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(l2_mask, gpu_numa_node,
			...
			/* Search for full-idle CPU on GPU's NUMA node with L3 cache affinity */
			if (l3_mask) {
				cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(l3_mask, gpu_numa_node,
							SCX_PICK_IDLE_CORE | __COMPAT_SCX_PICK_IDLE_IN_NODE);
				if (cpu >= 0) {
					*is_idle = true;
					goto out_put_cpumask;
			  ...
			  s32 prev_cpu, bool idle_smt)
{
	 ...
	 * For per-node DSQ mode, restrict search to the same NUMA node.
	 * For per-CPU and shared DSQ modes, allow search across all nodes.
	 */
	if (dsq_mode == DSQ_MODE_NODE)
		flags |= __COMPAT_SCX_PICK_IDLE_IN_NODE;
	...
	if (mask) {
		cpu = __COMPAT_scx_bpf_pick_idle_cpu_node(mask, node, flags);
		if (cpu >= 0) {
				...
				s32 prev_cpu, u64 slice, u64 enq_flags)
{
	...
	if (!__COMPAT_is_enq_cpu_selected(enq_flags)) {
		...
		/*
		 * GPU-aware optimization: For inference workloads (latency-sensitive),
		 * prioritize direct dispatch to reduce scheduling overhead.
		 */
		if (gpu_awareness_enabled && is_inference_workload(tctx)) {
			/* For inference, prefer GPU-local CPUs for direct dispatch */
			if (is_gpu_local_cpu(prev_cpu) && scx_bpf_test_and_clear_cpu_idle(prev_cpu)) {
				scx_bpf_dsq_insert(p, SCX_DSQ_LOCAL, slice_max, enq_flags);
				__sync_fetch_and_add(&nr_direct_dispatches, 1);
				return true;
			}
...
void BPF_STRUCT_OPS(bpfland_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	/*
	 * GPU-aware time slice adjustment: Optimize slice duration based on workload type.
	 */
	if (gpu_awareness_enabled) {
		if (is_inference_workload(tctx)) {
			/* Inference workloads: shorter slices for better responsiveness */
			slice = CLAMP(slice / 2, slice_min, slice_max);
		} else if (is_training_workload(tctx)) {
			/* Training workloads: longer slices for better throughput */
			slice = CLAMP(slice * 2, slice_min, slice_max);
		}
	}
		   ...
		   struct scx_init_task_args *args)
{
	...
	/*
	 * Initialize GPU-aware fields for new tasks.
	 */
	if (gpu_awareness_enabled) {
		tctx->gpu_workload_type = GPU_WORKLOAD_NONE;
		tctx->gpu_memory_usage = 0;
		tctx->last_gpu_access = 0;
		tctx->gpu_affinity_set = false;
		tctx->gpu_numa_node = gpu_numa_node;
		tctx->gpu_access_frequency = 0;
	}
...
s32 BPF_STRUCT_OPS_SLEEPABLE(bpfland_init)
{
	...
	case DSQ_MODE_NODE:
	default:
		/* Create per-NUMA-node DSQs (default) */
		bpf_for(node, 0, __COMPAT_scx_bpf_nr_node_ids()) {
			err = scx_bpf_create_dsq(node, node);
			if (err) {
				scx_bpf_error("failed to create per-node DSQ %d: %d", node, err);
				return err;
			}
		}
		break;
	...
	/* Do not update NUMA statistics if there's only one node or DSQ mode is not per-node */
	if (numa_disabled || dsq_mode != DSQ_MODE_NODE || __COMPAT_scx_bpf_nr_node_ids() <= 1)
		return 0;
 ...
 */
struct gpu_ctx gpu_context;

/*
 * GPU-related configuration and state.
 */
const volatile bool gpu_awareness_enabled = true;
const volatile u32 gpu_numa_node = 0;  /* Default to node 0, will be set during init */
const volatile u64 gpu_memory_total = 0; /* Will be set during init */

/*
 * GPU memory pressure tracking.
 */
volatile u64 gpu_memory_used;
volatile u64 gpu_compute_utilization;
