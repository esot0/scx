		...
		s32 prev_cpu, u64 wake_flags)
{
	...
	*/
	if (stay_with_kthread && cctx && cctx->has_active_kthread) {
		...
		dbg_msg("spark_select_cpu: Previous task is a per-CPU kthread, inserting into Local DSQ. Task: %s", p->comm);
...
if(stay_with_kthread){ 
	 /* This won't always necessarily be the thread preempted by a kthread, may need to modify logic.
	 * Consider the case where a task T1 sleeps, then another task, T2, runs on the same CPU.
	 * Then, a per-cpu kthread T3 preempts T2. While it's running, T1 may wake up and enqueue, making it likely to be the next task to run.
	 * However, the desired behavior for this case is to have T2 run next instead. 
	 * Note: Following desired behavior, could T2 starve T1?
	*/
	cctx = try_lookup_cpu_ctx(prev_cpu);
	if (cctx && cctx->has_active_kthread) {
		dbg_msg("spark_enqueue: Previous task is a per-CPU kthread, inserting into Local DSQ", p->comm);
		scx_bpf_dsq_insert(p, SCX_DSQ_LOCAL, slice, enq_flags);
...
void BPF_STRUCT_OPS(spark_running, struct task_struct *p)
{
	...
	if (stay_with_kthread && is_kthread(p) && p->nr_cpus_allowed == 1) {
		dbg_msg("spark_running: task=%s is a kthread, setting kthread activity flag", p->comm);
		struct cpu_ctx *cctx = try_lookup_cpu_ctx(cpu);
		if (cctx) {
...
void BPF_STRUCT_OPS(spark_stopping, struct task_struct *p, bool runnable)
{
	...
	// if (stay_with_kthread && is_kthread(p) && p->nr_cpus_allowed == 1) {
	// 	dbg_msg("spark_stopping: task=%s is a kthread, resetting kthread activity flag", p->comm);
	// 	cctx = try_lookup_cpu_ctx(cpu);
	// 	if (cctx) {
	// 		cctx->has_active_kthread = false; 
	// 	}
