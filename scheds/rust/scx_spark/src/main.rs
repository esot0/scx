...
struct Opts {
    ...
    /// Enable GPU awareness for single-GPU optimization.
    ///
    /// This enables GPU-aware scheduling optimizations for AI workloads,
    /// including NUMA locality, priority boosting, and workload-specific
    /// time slice adjustments.
    #[clap(long, action = clap::ArgAction::SetTrue)]
    gpu_awareness: bool,

    /// GPU NUMA node ID (0 = auto-detect).
    ///
    /// Specifies which NUMA node contains the GPU. Set to 0 for auto-detection.
    #[clap(long, default_value = "0")]
    gpu_numa_node: u32,

    /// GPU memory size in GB (0 = auto-detect).
    ///
    /// Specifies the total GPU memory size for memory pressure tracking.
