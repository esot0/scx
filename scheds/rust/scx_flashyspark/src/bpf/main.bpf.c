 ...
 *
 * Values between the two thresholds retain the current smoothed performance
 * level.
 */
...
const volatile bool more_dsqs;

const volatile bool stay_with_short_exec_runtime;
...
struct cpu_ctx {
    ...
    bool has_active_short_exec_runtime;
};
 ...
 */
static inline bool is_kthread(const struct task_struct *p) { return p->flags & PF_KTHREAD; }
 ...
 */
static bool is_queued(const struct task_struct *p) { return p->scx.flags & SCX_TASK_QUEUED; }
...
static s32 pick_idle_cpu_node(const struct cpumask *cpus_allowed, int node, u64 flags) {
    return numa_disabled ? scx_bpf_pick_idle_cpu(cpus_allowed, flags)
 ...
 */
static inline u64 node_to_dsq(int node) { return DSQ_FLAG_NODE | node; }
...
static inline u64 nr_tasks_waiting(s32 cpu) {
    ...
    return scx_bpf_dsq_nr_queued(cpu_to_dsq(cpu)) + scx_bpf_dsq_nr_queued(node_to_dsq(node));
}
...
static void task_update_domain(struct task_struct *p, struct task_ctx *tctx, s32 cpu,
    ...
    /*
     * For GPU tasks in aggressive mode, restrict to big CPUs if available
     */
    if (aggressive_gpu_tasks && tctx->is_gpu_task && big_cpumask) {
        bpf_cpumask_copy(mask, cast_mask(big_cpumask));
 ...
 */
static s32 pick_idle_cpu_builtin(struct task_struct *p, s32 prev_cpu, u64 wake_flags,
                                 bool *is_idle) {
    ...
    cpu = primary_all ? -ENOENT : scx_bpf_select_cpu_and(p, prev_cpu, wake_flags, primary, 0);
    if (cpu < 0) {
...
static s32 pick_idle_big_cpu(struct task_ctx *tctx, s32 prev_cpu, u64 wake_flags, bool *is_idle,
                             int *cpu) {
...
static s32 pick_idle_little_cpu(struct task_ctx *tctx, s32 prev_cpu, u64 wake_flags, bool *is_idle,
                                int *cpu) {
 ...
 * scheduling overhead.
 */
static s32 pick_idle_cpu(struct task_struct *p, struct task_ctx *tctx, s32 prev_cpu, u64 wake_flags,
                         bool *is_idle) {
     ...
     */
    is_prev_allowed =
        (primary_all || sticky_cpu) ? true : p_mask && bpf_cpumask_test_cpu(prev_cpu, p_mask);
    ...
    if (is_wake_sync(current, prev_cpu, this_cpu, wake_flags)) {
         ...
         * pipeline).
         */
        if ((share_llc || current->pid == tctx->waker_pid) && p_mask &&
            bpf_cpumask_test_cpu(this_cpu, p_mask) &&
            !scx_bpf_dsq_nr_queued(SCX_DSQ_LOCAL_ON | this_cpu) &&
            !scx_bpf_dsq_nr_queued(cpu_to_dsq(this_cpu)) && !is_llc_busy(this_cpu)) {
            cpu = this_cpu;
            *is_idle = true;
            goto out_put_cpumask;
        }
    ...
    if (smt_enabled) {
         ...
         */
        if (is_prev_allowed && bpf_cpumask_test_cpu(prev_cpu, idle_smtmask) &&
            scx_bpf_test_and_clear_cpu_idle(prev_cpu)) {
            cpu = prev_cpu;
            *is_idle = true;
            goto out_put_cpumask;
        ...
        if (p_mask != p->cpus_ptr) {
            cpu = pick_idle_cpu_node(p->cpus_ptr, node, SCX_PICK_IDLE_CORE);
            if (cpu >= 0) {
                *is_idle = true;
                goto out_put_cpumask;
            }
     ...
     */
    if (is_prev_allowed && scx_bpf_test_and_clear_cpu_idle(prev_cpu)) {
        cpu = prev_cpu;
        *is_idle = true;
        goto out_put_cpumask;
    }
 ...
 */
s32 BPF_STRUCT_OPS(flashyspark_select_cpu, struct task_struct *p, s32 prev_cpu, u64 wake_flags) {
    struct task_ctx *tctx;
    bool is_idle = false;
    s32 cpu;
    struct cpu_ctx *cctx;
	...
	else if(stay_with_short_exec_runtime && cctx->has_active_short_exec_runtime) {
		cctx->has_active_short_exec_runtime = false;
		dbg_msg("spark_select_cpu: Previous task is a short exec_runtime task, inserting "
			"into Local DSQ. Task: %s",
			p->comm);
	}
 ...
 */
static bool kick_idle_cpu(const struct task_struct *p, const struct task_ctx *tctx, s32 prev_cpu,
                          bool idle_smt) {
 ...
 */
static bool try_direct_dispatch(struct task_struct *p, struct task_ctx *tctx, s32 prev_cpu,
                                u64 enq_flags) {
 ...
 */
static void rr_enqueue(struct task_struct *p, struct task_ctx *tctx, s32 prev_cpu, u64 enq_flags) {
    bool is_idle;
    s32 cpu;
...
void BPF_STRUCT_OPS(flashyspark_enqueue, struct task_struct *p, u64 enq_flags) {
    ...
    if (get_stay_with_kthread() && cctx) {
        ...
        if (cctx->has_active_kthread) {
            dbg_msg("spark_enqueue: Previous task is a per-CPU kthread or has short exec_runtime, inserting "
                    "into Local DSQ",
                    p->comm);
            scx_bpf_dsq_insert(p, SCX_DSQ_LOCAL, task_slice(prev_cpu), enq_flags);
            cctx->has_active_kthread = false;
    ...
    if (stay_with_short_exec_runtime && cctx) {
	dbg_msg("spark_enqueue: Previous task has short exec_runtime, inserting "
		"into Local DSQ",
		p->comm);
	scx_bpf_dsq_insert(p, SCX_DSQ_LOCAL, task_slice(prev_cpu), enq_flags);	
	cctx->has_active_short_exec_runtime = false;
...
//             // if turbos are taking too long? Also not sure this is
//             // the right slice for now
//             goto workload_statistics;
//         } else if (aggressive_gpu_tasks &&
//                    p->nr_cpus_allowed > 1) { // Non-GPU tasks that aren't per-cpu threads in
//             // aggressive mode, insert into the little queue
//             scx_bpf_dsq_insert_vtime(p, LITTLE_DSQ_ID, slice_max, p->scx.dsq_vtime, enq_flags);
//             goto workload_statistics;
//         }

//         if (is_kthread(p) && is_pcpu_task(p)) { // aggressive gpu task mode can starve kthreads, so
//             // directly dispatch
//             scx_bpf_dsq_insert(p, SCX_DSQ_LOCAL, task_slice(prev_cpu), enq_flags);
//             goto workload_statistics;
//         }
//     }
    ...
    if (!scx_bpf_task_running(p) && can_enqueue_to_cpu(p, prev_cpu)) {
        scx_bpf_dsq_insert_vtime(p, cpu_to_dsq(prev_cpu), task_slice(prev_cpu), p->scx.dsq_vtime,
                                 enq_flags);
    ...
    scx_bpf_dsq_insert_vtime(p, node_to_dsq(node), task_slice(prev_cpu), p->scx.dsq_vtime,
                             enq_flags);
    __sync_fetch_and_add(&nr_shared_dispatches, 1);
...
//             scx_bpf_dsq_move_to_local(dsq_id) || scx_bpf_dsq_move_to_local(BIG_DSQ_ID) ||
//             scx_bpf_dsq_move_to_local(LITTLE_DSQ_ID)) {
//             return;
//         } else if ((cctx->is_big && scx_bpf_dsq_move_to_local(BIG_DSQ_ID)) ||
//                    scx_bpf_dsq_move_to_local(dsq_id) || scx_bpf_dsq_move_to_local(LITTLE_DSQ_ID)) {
//             return;
//         } else if (scx_bpf_dsq_move_to_local(dsq_id) || scx_bpf_dsq_move_to_local(LITTLE_DSQ_ID)) {
//             return;
//         }
//     }
...
void BPF_STRUCT_OPS(flashyspark_running, struct task_struct *p) {
    ...
    if(stay_with_short_exec_runtime && tctx->exec_runtime <= slice_max / 100) {
        ...
        if(cctx) {
            cctx->has_active_short_exec_runtime = true;
        }
    }
...
void BPF_STRUCT_OPS(flashyspark_set_cpumask, struct task_struct *p, const struct cpumask *cpumask) {
...
s32 BPF_STRUCT_OPS(flashyspark_init_task, struct task_struct *p, struct scx_init_task_args *args) {
    s32 cpu = bpf_get_smp_processor_id();
    struct task_ctx *tctx;
    int err;
...
static int numa_timerfn(void *map, int *key, struct bpf_timer *timer) {
    ...
    bpf_for(node, 0, __COMPAT_scx_bpf_nr_node_ids()) {
        ...
        dbg_msg("node %d util %llu rebalance %d", node, nctx->perf_lvl, nctx->need_rebalance);
    }
 ...
 * Exit the scheduler.
 */
void BPF_STRUCT_OPS(flashyspark_exit, struct scx_exit_info *ei) { UEI_RECORD(uei, ei); }

SCX_OPS_DEFINE(flashyspark_ops, .select_cpu = (void *)flashyspark_select_cpu,
               .enqueue = (void *)flashyspark_enqueue, .dispatch = (void *)flashyspark_dispatch,
               .running = (void *)flashyspark_running, .stopping = (void *)flashyspark_stopping,
