...
#[command(
    ...
    long_about = r#"
scx_flashyspark is scheduler that focuses on ensuring fairness and performance predictability.
...
impl<'a> Scheduler<'a> {
    fn init(opts: &'a Opts, open_object: &'a mut MaybeUninit<OpenObject>) -> Result<Self> {
        ...
        let mut skel = scx_ops_open!(skel_builder, open_object, flashyspark_ops)?;

        skel.struct_ops.flashyspark_ops_mut().exit_dump_len = opts.exit_dump_len;
        ...
        // Set scheduler flags.
        skel.struct_ops.flashyspark_ops_mut().flags = *compat::SCX_OPS_ENQ_EXITING
        ...
        info!(
            "scheduler flags: {:#x}",
            skel.struct_ops.flashyspark_ops_mut().flags
        );

        // Load the BPF program for validation.
        let mut skel = scx_ops_load!(skel, flashyspark_ops, uei)?;

        if !opts.aggressive_gpu_tasks {
            unsafe {
    ...
    fn init_energy_domain(skel: &mut BpfSkel<'_>, domain: &Cpumask) -> Result<()> {
        ...
        if let Err(err) = Self::enable_cpu(skel, -1, 0) {
        ...
        for cpu in 0..*NR_CPU_IDS {
            if domain.test_cpu(cpu) {
                if let Err(err) = Self::enable_cpu(skel, cpu as i32, 0) {
    ...
    fn init_smt_domains(skel: &mut BpfSkel<'_>, topo: &Topology) -> Result<(), std::io::Error> {
        ...
        for (cpu, sibling_cpu) in smt_siblings.iter().enumerate() {
            Self::enable_sibling_cpu(skel, 0, cpu, *sibling_cpu as usize, 0).unwrap();
        }
    ...
    fn init_cache_domains(
        ...
        topo: &Topology,
        cache_lvl: usize,
        enable_sibling_cpu_fn: &dyn Fn(&mut BpfSkel<'_>, usize, usize, usize, usize) -> Result<(), u32>,
    ) -> Result<(), std::io::Error> {
        // Determine the list of CPU IDs associated to each cache node.
        let mut cache_id_map: BTreeMap<usize, (Vec<usize>, Vec<usize>, Vec<usize>)> = BTreeMap::new(); //big cpus, little cpus, all cpues
        ...
        // Update the BPF cpumasks for the cache domains.
            for (cache_id, (big_cpus, little_cpus, all_cpus)) in cache_id_map {
                ...
                if Self::are_smt_siblings(topo, &all_cpus) {
                    continue;
                }
